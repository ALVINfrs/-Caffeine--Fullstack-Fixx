...
export default function TransactionSuccessPage() {
  ...
  useEffect(() => {
    ...
    const loadTransactionData = () => {
      ...
      if (storedData) {
        try {
          ...
          console.log("Transaction data loaded from session:", parsedData);
        } catch (error) {
          console.error("Error parsing transaction data from session:", error);
        }
      }
      // If no session data, try to get data from localStorage based on URL param
        ...
        if (localData) {
          try {
            ...
            setTransactionData(parsedData);
            console.log(
              "Transaction data loaded from localStorage:",
              parsedData
            );
          } catch (error) {
            console.error(
    ...
    // Don't remove from sessionStorage immediately to allow redirects to work
    // Instead, use a small timeout to clear the session data after we've stored it in localStorage
  ...
  const getPaymentMethodName = (method: string) => {
    ...
    const methods: Record<string, string> = {
      ...
      shopeepay: "ShopeePay",
      // Add other Midtrans payment methods as needed
    };
